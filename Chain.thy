...
next
  ...
  hence y_freshC: "y \<sharp> C" and yvec_freshp: "yvec \<sharp>* C" by simp+
  have "(x' # xvec) \<sharp>* (y # yvec)" by fact
  hence x'ineqy: "x' \<noteq> y" and xvec_freshyvec: "xvec \<sharp>* yvec"
    and x'_freshyvec: "x' \<sharp> yvec" and y_freshxvec: "y \<sharp> xvec"
    by(auto simp add: fresh_list_cons)
  have "distinct (y#yvec)" by fact
  hence y_freshyvec: "y \<sharp> yvec" and yvec_dist: "distinct yvec"
    by simp+
  have L: "length xvec = length yvec" by fact
  have "x \<sharp> [(x', y)] \<bullet> [xvec yvec] \<bullet>\<^sub>v C"
  proof(case_tac "x = x'")
    assume xeqx': "x = x'"
    moreover from y_freshxvec y_freshyvec y_freshC L have "y \<sharp> [xvec yvec] \<bullet>\<^sub>v C"
      by simp
  ...
  next
    ...
    ultimately have "x \<sharp> [xvec yvec] \<bullet>\<^sub>v C" using yvec_freshp xvec_freshyvec yvec_dist
      by simp
    hence "([(x', y)] \<bullet> x) \<sharp> [(x', y)] \<bullet> [xvec yvec] \<bullet>\<^sub>v C"
      by(rule pt_fresh_bij1[OF pt_name_inst, OF at_name_inst])
    moreover from xmemxvec y_freshxvec have "x \<noteq> y"
...
next
  ...
  hence y_freshp: "y \<sharp> p" and yvec_freshp: "yvec \<sharp>* p"
    by simp+
  moreover have "(x # xvec) \<sharp>* (y # yvec)" by fact
  hence xineqy: "x \<noteq> y" and xvec_freshyvec: "xvec \<sharp>* yvec"
  ...
  with yvec_freshp xvec_freshyvec yvec_dist have IH: "xvec \<sharp>* ([xvec yvec] \<bullet>\<^sub>v p)" by simp
  show ?case
  proof(auto)
    from L y_freshp yvec_freshp xineqy xvec_freshyvec yvec_dist y_freshyvec y_freshxvec x_freshyvec
    have "x \<sharp> [(x # xvec) (y # yvec)] \<bullet>\<^sub>v p"
      by(rule_tac fresh_chain_perm) (auto simp add: fresh_list_cons)
  ...
  next
    ...
    proof(case_tac "x mem xvec")
      assume "x mem xvec"
      with L yvec_freshp xvec_freshyvec yvec_dist x_freshyvec
      have"x \<sharp> [xvec yvec] \<bullet>\<^sub>v p"
	by(rule_tac fresh_chain_perm) (auto simp add: fresh_list_cons)
      moreover from y_freshxvec y_freshyvec y_freshp L
      have "y \<sharp> [xvec yvec] \<bullet>\<^sub>v p" by simp
      ultimately show ?thesis using IH
	by(subst cons_perm) (simp add: perm_fresh_fresh)
    next
      assume "\<not>(x mem xvec)"
      hence x_freshxvec: "x \<sharp> xvec" by simp
      from IH have "([(x, y)] \<bullet> xvec) \<sharp>* ([(x, y)] \<bullet> [xvec yvec] \<bullet>\<^sub>v p)"
	by(simp add: pt_fresh_star_bij[OF pt_name_inst, OF at_name_inst])
      with x_freshxvec y_freshxvec show ?thesis by simp
    qed
  qed
qed
...
using assms
apply(induct rule: compose_perm.induct, auto)
by(subst pt_perm_compose[OF pt_name_inst, OF at_name_inst]) simp

lemma distinct_perm_closed[simp]:
  fixes p :: "name prm"
  and   q :: "name prm"

  assumes "distinct_perm p"

  shows "distinct_perm(q \<bullet> p)"
using assms
by(induct p) (auto simp add: pt_fresh_bij[OF pt_name_inst, OF at_name_inst] dest: pt_bij4[OF pt_name_inst, OF at_name_inst])

lemma fresh_star_simps:
  fixes x  :: name
  and   Xs :: "name set"
  and   Ys :: "name set"
  and   C  :: "'a::fs_name"
...
using assms
by(induct xvec) (auto simp add: fresh_star_simps)

lemma perm_star_fresh:
  fixes xvec :: "name list"
  and   p    :: "name prm"
  and   T    :: "'a::pt_name"
...
using assms
by(induct p) (auto simp add: chain_fresh_fresh)

lemma swap_star_fresh:
  fixes x :: name
  and   p :: "name prm"
  and   T :: "'a::pt_name"
...
proof -
  from assms have "[x] \<sharp>* (p \<bullet> T) = [x] \<sharp>* T"
    by(rule_tac perm_star_fresh) auto
  thus ?thesis by simp
qed

lemmas fresh_chain_simps = fresh_star_simps fresh_star_chain_simps perm_star_fresh swap_star_fresh chain_fresh_fresh fresh_perm subset_fresh

lemma fresh_alpha_perm:
  fixes xvec :: "name list"
  and   Xs   :: "name set"
  and   Ys   :: "name set"
  and   p    :: "name prm"
...
proof -
  from assms have "[x] \<sharp>* p"
    by (metis fresh_alpha_perm fresh_star_list_cons(2) fresh_star_list_nil(2))
  thus ?thesis by simp
qed

lemma set_to_list_fresh[simp]:
  fixes xvec :: "name list"
  and   C    :: "'a::fs_name"
  and   yvec :: "name list"
  and   Xs   :: "name set"
